...
public class K_GrandpaPatrol : K_FSMHelper
{
    /*
     * Patrol around map going from waypoint to waypoint, often times a snowfort. Upon detecting the player
     * transition to the Defense and hold the currently closet fort.
     */
    ...
    //Above line taken out because of the Helper class. NPCFSMBaseHelper.cs

    GameObject[] waypoints; // Places to move the NPC towards and away, tried list, didn't understand to how properly set up
    [SerializeField]int currentWaypoint; //Current Waypoint to which the NPC will move towards
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        timer = 90f; //Set timer to a new value of 30f, the time it takes to complete a full patrol
    }
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (Vector3.Distance(waypoints[currentWaypoint].transform.position, NPC.transform.position) < 1f)
        {
        ...
        //Change Behaviour back to idle if 90 seconds pass without contact with the player, reset Patrol
        if (timer <= 0)
        {
